...
class A2C(AbstractSolver):
    ...
    def train(self, states, actions, rewards, next_state, done):
        ...
        next_value = 0.0 if done else self.actor_critic.value(torch.tensor(next_state, dtype=torch.float32))
        for t in reversed(range(len(rewards))):
            next_value = rewards[t] + self.options.gamma * next_value
        ...
        critic_loss = 0.0
         # Compute actor and critic losses
        policy_loss = -torch.sum(self.actor_critic.log_probs(states_tensor) * actions_one_hot * advantages.detach())
